...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        ...
        if len(actions) > 0:
          for a in actions:
              qValuesAction[a] = self.getQValue(state,a)
          return qValuesAction[qValuesAction.argMax()]
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        for k in feat.keys():
            self.weights[k] = self.weights[k] + feat[k] * self.alpha * diff
