    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance != None:
            for pos in self.legalPositions:
                trueDist = util.manhattanDistance(pos, pacmanPosition)
                allPossible[pos] =  self.beliefs[pos] * emissionModel[trueDist]
        else:
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for b in self.beliefs:
            if self.beliefs[b] > 0:
                self.setGhostPosition(gameState, b)
                gDist = self.getPositionDistribution(gameState)
        
                for p in self.legalPositions:
                    newBel[p] = newBel[p] + gDist[p] * self.beliefs[b]
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance != None:            
            belOld = self.getBeliefDistribution()

            for pos in self.legalPositions:
                dist = util.manhattanDistance(pos, pacmanPosition)
                allPos[pos] = emissionModel[dist] * belOld[pos]
            
            if not any(allPos.values()):
                self.initializeUniformly(gameState)
            
            else:                
                for n in range(self.numParticles):
                    partNew.append(util.sample(allPos))

                self.particles = partNew
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        while self.numParticles > c:
            for p in pos:
                if self.numParticles > c:
                    c = c + 1
    ...
    def observeState(self, gameState):
        ...
        for p in self.particles:
            ...
            for n in range(self.numGhosts):
                if noisyDistances[n] != None:
                    pt = pt * emissionModels[n][util.manhattanDistance(p[n], pacmanPosition)]

                else:
                    p = self.getParticleWithGhostInJail(p, n)
                    
            part[p] = part[p] + pt

        if any(part.values()):
            part.normalize()
            self.particles = [util.sample(part) for m in range(self.numParticles)]
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            """
            for n in range(self.numGhosts):                
                newParticle[n] = util.sample(getPositionDistributionForGhost(setGhostPositions(gameState, newParticle), n, self.ghostAgents[n]))
            """
            for i in range(self.numGhosts):
    ...
    def getBeliefDistribution(self):
        "*** YOUR CODE HERE ***"
        belDist = util.Counter()

        for p in self.particles:
            belDist[p] = belDist[p] + 1
