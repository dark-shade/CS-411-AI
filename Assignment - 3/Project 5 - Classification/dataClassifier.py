...
def enhancedFeatureExtractorDigit(datum):
        ...
        for m in range(1,len(pxs[l])):
            if pxs[l][m] != 0:                
                ...
                if not fl or m < fl:
                    fl = m
                nzCnt = nzCnt + 1
            if pxs[l][m - 1] != pxs[l][m]:
                bCnt = bCnt + 1

    ft = None
    pr = 0
    for m in range(len(pxs[0])):
        col = []
        for px in pxs:
            col.append(px[m])
    ...
    for n in range(5):
        features[n] = bCnt > 175 and 1.0 or 0.0

    for n in range(10):
        features[(n + 1) * 10] = float(len(pxs[0]) - (fl * 2)) / (len(pxs) - (ft * 2)) < 0.69

    for n in range(5):
        features[-n] = nzCnt > 300 and 1.0 or 0.0
    
    for n in range(5):
        features[-(n + 1) * 10] = float(ac) / nzCnt > 0.35 and 1.0 or 0.0

    for n in range(1000, 1005):
        features[n] = float(pr) / nzCnt < 0.27 and 1.0 or 0.0
...
def enhancedPacmanFeatures(state, action):
    ...
    features = util.Counter()
    "*** YOUR CODE HERE ***"
    nextOne = state.generateSuccessor(0, action)
    foods = nextOne.getFood().asList()
    for f in foods:
        features[str(f)] = 1 / util.manhattanDistance(f, nextOne.getPacmanPosition())
